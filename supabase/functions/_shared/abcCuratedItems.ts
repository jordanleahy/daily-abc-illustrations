/**
 * Curated ABC items for themed alphabet books (Edge Function shared module)
...
/**
 * Format curated items as a reference string for prompt injection
 */
export function getCuratedItemsList(themeKey: string): string {
  const items = ABC_CURATED_ITEMS[themeKey as ABCThemeType];
  if (!items) return '';
 ...
 */
export function isValidABCTheme(themeKey: string): themeKey is ABCThemeType {
  return themeKey in ABC_CURATED_ITEMS;
}
