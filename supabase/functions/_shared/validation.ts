...
// ============= Rhyming Book Validation =============

/**
 * Validates Rhyming book outline structure
 * Ensures Page 1 is Cover, Page 2 is Educational Focus, and all 12 pages are present
 */
export function validateRhymingBookStructure(
  pages: any[]
): {
  valid: boolean;
  errors: string[];
  ...
  // Check page count (should be 12 total)
  if (pages.length !== 12) {
    errors.push(`Expected 12 pages, got ${pages.length}`);
  }

  // Validate Page 1 is Cover
  const page1 = pages.find(p => p.pageNumber === 1);
  if (!page1) {
    errors.push('Missing Page 1 (Cover)');
  } else {
    const title1 = (page1.title || '').toLowerCase();
    if (!title1.includes('cover') && page1.pageType !== 'cover') {
      errors.push(`Page 1 must be Cover page. Got: "${page1.title}" with pageType: "${page1.pageType}"`);
    }
  }

  // Validate Page 2 is Educational Focus
  const page2 = pages.find(p => p.pageNumber === 2);
  if (!page2) {
    errors.push('Missing Page 2 (Educational Focus)');
  } else {
    const title2 = (page2.title || '').toLowerCase();
    const desc2 = (page2.description || '').toLowerCase();
    const hasEducationalKeywords = 
      title2.includes('educational') || 
      title2.includes('focus') || 
      desc2.includes('badge') ||
      page2.pageType === 'educational';
    
    if (!hasEducationalKeywords) {
      errors.push(`Page 2 must be Educational Focus with badges. Got: "${page2.title}"`);
    }
  }

  // Check that content pages start at Page 3
  const contentPages = pages.filter(p => p.pageNumber >= 3);
  if (contentPages.length !== 10) {
    warnings.push(`Expected 10 content pages (3-12), got ${contentPages.length}`);
  }

  // Validate rhyming couplet format for content pages
  contentPages.forEach(page => {
    const title = page.title || '';
    // Rhyming titles should contain "/" or "," indicating a couplet
    const hasCoupletFormat = title.includes('/') || title.includes(',') || title.split('\n').length > 1;
    if (!hasCoupletFormat && !title.toLowerCase().includes('rhyme')) {
      warnings.push(`Page ${page.pageNumber}: Title may not be in rhyming couplet format: "${title.substring(0, 50)}..."`);
    }
  });
...
/**
 * Quick check if outline follows standard page structure
 * Returns true if Page 1 appears to be Cover and Page 2 appears to be Educational
 */
export function hasValidPageStructure(pages: any[]): boolean {
  ...
  const page1IsCover = 
    page1.pageType === 'cover' ||
    (page1.title || '').toLowerCase().includes('cover');
  
  const page2IsEducational = 
    page2.pageType === 'educational' ||
  ...
  return page1IsCover && page2IsEducational;
}
