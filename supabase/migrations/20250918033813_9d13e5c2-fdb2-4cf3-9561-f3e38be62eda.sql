-- Update expiration time from 48 hours to 24 hours
...
-- 2. Update queued items to have 24-hour intervals instead of 48-hour
UPDATE public.daily_published 
SET 
  published_at = (SELECT published_at FROM daily_published WHERE queue_position = 1) + INTERVAL '24 hours' * (queue_position - 1),
  expires_at = (SELECT published_at FROM daily_published WHERE queue_position = 1) + INTERVAL '24 hours' * queue_position
WHERE status = 'queued';
