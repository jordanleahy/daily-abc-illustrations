-- Add publish_date column to daily_published table
ALTER TABLE public.daily_published 
ADD COLUMN publish_date DATE NOT NULL DEFAULT CURRENT_DATE;

-- Add unique constraint: only one active book per date
ALTER TABLE public.daily_published 
ADD CONSTRAINT unique_active_per_date 
UNIQUE (publish_date, status) 
DEFERRABLE INITIALLY DEFERRED;

-- Create index for efficient date-based queries
CREATE INDEX idx_daily_published_publish_date_status 
...
-- Update existing records to have proper publish_date
UPDATE public.daily_published 
SET publish_date = CASE 
  WHEN status = 'active' THEN CURRENT_DATE
  WHEN status = 'expired' THEN CURRENT_DATE - INTERVAL '1 day'
