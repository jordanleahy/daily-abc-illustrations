...
-- Create partial unique index: only one active item per date
CREATE UNIQUE INDEX IF NOT EXISTS unique_active_per_date 
ON public.daily_published (publish_date) 
WHERE status = 'active';
