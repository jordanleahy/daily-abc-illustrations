-- Simplify back to core daily publishing: queue order determines sequence, 7:01 AM daily activation

-- Remove the complex date/time columns that are causing confusion
...
-- Add a simple queue_order column for drag-and-drop ordering
ALTER TABLE daily_published 
ADD COLUMN IF NOT EXISTS queue_order INTEGER;

-- Set queue_order based on current publish_date for existing records (using CTE to avoid window function in UPDATE)
WITH ordered_items AS (
  SELECT id, ROW_NUMBER() OVER (ORDER BY publish_date ASC, created_at ASC) as new_queue_order
  FROM daily_published 
  WHERE queue_order IS NULL
)
UPDATE daily_published 
SET queue_order = ordered_items.new_queue_order
FROM ordered_items 
WHERE daily_published.id = ordered_items.id;

-- Simplify the database function back to core logic: activate next in queue daily at 7:01 AM
...
BEGIN
  ...
  IF activated_item_id IS NOT NULL THEN
    ...
    SET 
      ...
      END,
      -- Set expires_at to 7:01 AM Eastern Time tomorrow (exactly 24 hours later)
      expires_at = CASE 
