-- Create function to manually activate a queued item and move current active to queue
CREATE OR REPLACE FUNCTION public.admin_manually_activate_item(
  p_item_id UUID
)
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  -- Activate the target item with proper Eastern Time handling
  UPDATE public.daily_published
  SET 
    status = 'active',
    is_active = true,
