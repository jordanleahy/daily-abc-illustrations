-- Fix unique constraint violation by updating pages in reverse order
CREATE OR REPLACE FUNCTION public.insert_page_at_position(
  p_book_id UUID,
...
BEGIN
  ...
  -- Shift pages in REVERSE order to avoid unique constraint violations
  ...
  LOOP
    UPDATE pages
    SET 
      page_number = v_page_record.page_number + 1,
      letter = v_alphabet[((v_page_record.page_number) % 26) + 1]
    WHERE id = v_page_record.id;
  END LOOP;
