-- Update ABC agent to use JSON Schema structured output format
UPDATE agents 
SET 
...
NEVER ask users for book length. Generate the full 26-letter set automatically once theme and letter case are selected.',
  what_changed = 'Updated ABC agent to use JSON Schema structured output format instead of old ```json...``` block format. All responses now output proper {"message": "...", "suggestions": [...]} structure required by the API-level schema enforcement.',
  last_modified = NOW(),
  version = 'v1.2.0',
