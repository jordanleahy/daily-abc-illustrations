-- Drop and recreate get_library_books_by_completion to include slug from daily_published
...
BEGIN
  ...
  book_slugs AS (
    ...
    SELECT DISTINCT ON (dp.book_id)
      dp.book_id,
      dp.slug
    FROM daily_published dp
    WHERE dp.slug IS NOT NULL
    ORDER BY dp.book_id, dp.created_at DESC
  )
  SELECT 
    b.id,
    bs.slug,
  ...
  LEFT JOIN book_slugs bs ON bs.book_id = b.id
