-- Simplify Manners agent instructions since discovery questions now come from type_specific_discoveries table
UPDATE agents
SET 
  instructions = '# Manners Book Creation Agent v1.4.0
...
5. Include all 12 pages with complete content for each',
  version = '1.4.0',
  updated_at = now(),
  what_changed = 'Simplified agent: Optional discovery questions now fetched from type_specific_discoveries table via database. Reduced instruction size by ~50%. Questions asked one at a time with Skip options.'
