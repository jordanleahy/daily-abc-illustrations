-- Update book-creation-rhyming agent to separate Image Prompt from text content
UPDATE agents 
SET 
...
- **Rhyme Text:** "[Full rhyming couplet with line breaks]"
- **Image Prompt:** [Art style]. [Visual scene description showing both rhyme elements]. [Character action and expression]. [Background/setting details]. Full frame composition. No text overlays. Clean illustration only.

### ✅ CORRECT Example:
**Page 3: The Cozy Cat**
- **Rhyme Pair:** mat / hat
- **Rhyme Text:** "The cat sat on a comfy mat,\nAnd wore a purple polka-dot hat."
- **Image Prompt:** Whimsical children''s book style. A fluffy orange tabby cat sitting contentedly on a cozy woven mat. The cat wears an oversized purple polka-dot party hat tilted playfully. Warm living room setting with soft lighting. Happy, relaxed expression. Full frame composition. No text overlays. Clean illustration only.

### ❌ WRONG (Never do this):
- **Image Prompt:** The cat sat on a mat... "Let''s play on the mat!" ...

### Image Prompt Rules:
1. ONLY describe what to DRAW visually
2. NEVER include dialogue, rhymes, or written words
3. NEVER include quotation marks in Image Prompt
4. ALWAYS end with: No text overlays. Clean illustration only.
5. Describe character actions that SHOW the rhyme visually',
  what_changed = 'CRITICAL FIX: Separated Image Prompt from Rhyme Text. Image prompts now contain ONLY visual descriptions - no rhyme text, no quotes, no words. Added explicit examples showing correct vs incorrect format. All prompts must end with "No text overlays. Clean illustration only." This prevents AI image generator from rendering text on images.'
